            ...
            <div class="w-full flex flex-wrap justify-center">
                @foreach( $drumPacks as $pack )
                    <a href="{{ route('pack.show', $pack) }}" class="w-2/5 h-25 max-w-xs max-h-md">
                        ...
                        <pack-component src="{{ asset('img/packs/drumEssentials2.jpeg') }}" name="{{ $pack->name }}" author="{{ $pack->author->name }}" :tags="{{ json_encode($pack->tags()->get()->pluck('name')->toArray())  }}" :amount="{{ $pack->amount }}"></pack-component>
                    </a>
                @endforeach
            </div>
        </div>
            ...
            <div class="w-full flex justify-around pb-2">
                ...
                <div class="w-full flex flex-wrap justify-center">
                    @foreach( $collections as $pack )
                        <a href="{{ route('pack.show', $pack) }}" class="w-2/5 h-25 max-w-xs max-h-md">
                            <pack-component src="{{ asset('img/packs/drumEssentials1.jpeg') }}" name="{{ $pack->name }}" author="{{ $pack->author->name }}" :tags="{{ json_encode($pack->tags()->get()->pluck('name')->toArray())  }}" :amount="{{ $pack->amount }}"></pack-component>
                        </a>
                    @endforeach
                </div>
            </div>
